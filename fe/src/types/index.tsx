...
export interface Voucher extends BaseEntity {
  ...
  isActive: boolean; // default: true
  usedCount?: number; // usage count from backend
  totalUsageLimit: number; // default: 1000 - total usage limit
...
// ============= 15.1. POPULATED WISHLIST TYPES =============
export interface PopulatedWishListItem {
  _id?: string;
  product: Product; // Populated Product object
  variant?: ProductVariant; // Populated ProductVariant object
  addedAt?: string; // Date when item was added (virtual field)
}

export interface PopulatedWishList extends BaseEntity {
  user: string; // ObjectId ref User, required, unique
  items: PopulatedWishListItem[];
}

// ============= 15.2. WISHLIST STATISTICS =============
...
export interface Banner extends BaseEntity {
  ...
  title?: string;
  description?: string; // optional - short description for banner info card
...
export interface CreateVariantRequest {
  product?: string; // Optional for form data, will be set by backend
  color: string;
  size: string;
  price: number;
  stock: number;
  sku?: string;
  images?: string[]; // Optional, will default to empty array
  isActive?: boolean;
}
...
export interface ProductFilters {
  ...
  order?: 'asc' | 'desc';
  isOnSale?: boolean | string; // Accept both boolean and string for backend compatibility
...
export interface CartContextType {
  ...
  addToCartSilent: (productVariantId: string, quantity: number) => Promise<void>;
...
export interface ProductWithCategory extends Omit<Product, 'category' | 'variants'> {
  category: Category;
  variants?: ProductVariantWithRefs[]; // Optional for admin views with populated refs
  stockInfo?: {
    totalStock: number;
    availableStock: number;
    reservedStock?: number;
  }; // From backend aggregation
}
